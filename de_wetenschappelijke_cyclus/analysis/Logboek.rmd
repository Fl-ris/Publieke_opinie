...
```{r Logistische regressie (man en vrouw) met leeftijd en geslacht als predictors}
# Logistische regressie met leeftijd en geslacht als predictors
logistische_regressie <- glm(eigen_keuze_abortus ~ Wat.is.uw.leeftijd. + geslacht, data = df_logistische_regressie, family = binomial)
logistische_regressie
```


``` {r Logistische regressie (man) met leeftijd, geloof en opleidingsniveau als predictors}
...
ggplot(data = lr_voorspelling_vader, aes(x = rank, y = lr_man_vader.fitted.values)) +
  geom_point(aes(color=df_man.vader, alpha = 0.9)) +
  ggtitle("Eigen keuze abortus:") +
  xlab("Index") +
  ylab("Waarschijnlijkheid (tussen 0 en 1)")
```
