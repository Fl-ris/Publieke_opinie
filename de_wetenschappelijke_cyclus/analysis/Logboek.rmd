...
#ggarrange(gezondheid_plot, vader_plot, echo_plot, bedenktijd_plot)
```
```{r}
# create a dataset
specie <- c(rep("sorgho" , 3) , rep("poacee" , 3) , rep("banana" , 3) , rep("triticum" , 3) )

keuzes <- df %>% select(geslacht, Wat.is.uw.leeftijd., levensgevaar_vrouw_o, geestelijke_handicap_o, levensbedreigende_ziekte_o, veroorzaakt_o, veroorzaakt_o, niet_gespecificeerd_o, financiele_beperkingen) %>% pivot_longer()
keuze <- df %>% select (rep(levensgevaar_vrouw_o),)  
...
keuzes <- df %>% select(geslacht, Wat.is.uw.leeftijd.) %>% pivot_wider(colls = c(1:2))
keuzes
