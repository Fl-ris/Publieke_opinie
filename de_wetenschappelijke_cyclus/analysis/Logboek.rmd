...
df_chi_geslacht <- (data.frame((chi_test_vector_geslacht)) %>% rename(
  ...
  chi_abortus_zeggenschap_ouders = p.value.3,
  chi_abortus_echo = p.value.4,
  chi_abortus_bedenktijd = p.value.5,
  chi_abortus_adoptie = p.value.6,
  chi_abortus_zeggenschap_vader = p.value.7
  ))
...
```
Aan de bovenstaande grafiek te zien is er alleen een significant verschil tussen de meningen van de verschillende geslachten voor de kwestie of de vader ook zeggenschap mag hebben over de abortus, dit is van alle vragen de vraag waar ik ook wel van had verwacht dat het een polarizerende vraag zou zijn. 
...
# Logischische regressie: #
Om het verband tussen meerdere predictors te zien is de logistische regressie gebruikt. 


```{r  Logistische regressie test}
# Nieuw dataframe met de relevante colommen van df.
df_logistische_regressie <- data.frame(ID = df$ID, geslacht =df$geslacht, leeftijd = df$Wat.is.uw.leeftijd., keuze = df$eigen_keuze_abortus)
...
Mogenlijk komt dit omdat er niet genoeg metingen zijn voor elk opleidingsniveau, daarom is opleidingsniveau weggehaald as predictor in het onderstaande code blok. 

```{r}

logistische_regressie <- glm(keuze ~ leeftijd + geslacht, data = df_logistische_regressie, family = binomial)

logistische_regressie

# Voorspel test met de volgende data:

newdata1 <- data.frame(ID = c(1,2), geslacht = c("Man", "Vrouw"), leeftijd = c(20,22), keuze = c("Wel abortus toestaan","Wel abortus toestaan"))
