...
abortus$geslacht <- factor(x = abortus$Wat.is.uw.geslacht..,
                           levels = c("Man", "Vrouw", "Wil ik niet vertellen"),
                           labels = c("Man", "Vrouw", "Wil ik niet vertellen"))
# Voor de vraag: Wat is uw religieuze overtuiging?
...
ggplot(df_1, aes(x= c(1:104), y = vader)) +
...
```
```{r}
df_2 <- data.frame(abortus)

ggplot(df_1, aes(x= ID, y = onderwijsniveau)) +
  geom_point(aes(color = geslacht)) +
  labs(color = "Geslacht") +
  ggtitle("Moet de vader zeggenschap hebben over \nde beslissing om een abortus te induceren? ")
...
ja_abortus <- sum(ifelse(abortus$eigen_keuze_abortus=="Wel abortus toestaan", 1,0)) # 1 voor de mensen de voorstander zijn, 0 als ze tegen zijn. 
