...
De weblink die verkregen was van Microsoft Forms, is middels digitale communicatie zoals: E-mail, whatsapp, discord etc. verspreid. 
...
```{r afkomst, warning=FALSE, fig.cap='Verdeling van de afkomst van de mensen die de enquete ingevuld hebben.', message=FALSE,}
...
```{r $chi-sq_2$, warning=FALSE, message=F, fig.cap='P-waarde score voor de enquete vragen en het effect dat geslacht (boven) en leeftijd (onder) hebben op het beantwoorden van de vragen.'}
...
```{r $barplot_gender_en_zeggenschap$, fig.cap='Verdeling van antwoorden over gender op de vraag of de biologische vader zeggenschap moet hebben over de beslissing voor een abortus.'}
...
En met de 51 mensen die we nu hebben is de effect sterkte: `r round(power1$d, digits = 2)`. De groep met het minst aantal "deelnemers", in dit geval de vrouwen moet gebruikt worden. Van deze significantie moet uitgegaan worden voor de analyzes. 


## **Log-odds logistische regressie:**


```{r odds ratio, warning=FALSE,results = "hide", message=FALSE}
...
#odds_ratio_lr_man_vrouw_vader <- data.frame(odds_ratio_lr_man_vrouw_vader) %>% row.names(
 # aaa = `(Intercept)`
#  )
#rownames <- rownames(odds_ratio_lr_man_vrouw_v`ader) 
