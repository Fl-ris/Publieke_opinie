---
title: Invloed van leeftijd, geslacht en opleidingsniveau op abortus gerelateerde ideologische verschillen in mening.
...
abstract: |
    Met behulp van een enquete is de mening over abortus gepeild onder een diverse groep mensen. Met de verkregen gegevens zijn verschillende analyses uitgevoerd om te achterhalen of factoren zoals: leeftijd, geslacht, geloof en opleidingsniveau leiden tot een andere mening over de abortus kwestie. Zoals in de bestaande literatuur al laten zien had, is er hier ook een significant verschil op te merken tussen de verschillende geslachten wanneer de vraag het zeggenschap van de biologische vader betreft. Het verschil tussen de geslachten was echter beperkt tot deze vraag. 
...
Alle gebruikte data en de scripts om deze data te verwerken zijn aanwezig in deze Github repository: \ https://github.com/Fl-ris/Publieke_opinie
...
```{r, eval=FALSE, warning=FALSE, message=FALSE, fig.cap='Vindt u dat adoptie een alternatief is voor abortus?', fig.asp=.75, out.width='70%', fig.align='center'}
...
```{r vader, eval=FALSE, warning=FALSE, message=FALSE, fig.cap='Moet de biologische vader zeggenschap hebben over de beslissing voor een abortus?', fig.asp=.75, out.width='70%', fig.align='center'}
...
```{r gezondheid, eval=FALSE, warning=FALSE, message=F, fig.cap='Antwoorden op de vraag: Vindt u dat er voor abortusgekozen mag worden als de gezondheid van de vrouw door de zwangerschap in gevaar kan komen?', fig.asp=.75, out.width='70%', fig.align='center'}
...
ggplot(data = gezondheid_vrouw_plot_data, mapping = aes(x = percentage, y = Var1, fill = Var2)) +
  ...
  theme_minimal()
```
```{r bedenktijd, eval=FALSE, warning=FALSE, message=F, fig.cap='Antwoorden op de vraag: Vindt u dat de wettelijke bedenktijd moet terugkomen?', fig.asp=.75, out.width='70%', fig.align='center'}
...
ggplot(data = bedenktijd_plot_data, mapping = aes(x = percentage, y = Var1, fill = Var2)) +
 ...
 # geom_text(aes(label = (""))) +
  ...
  theme_minimal()
```
```{r levensgevaar, warning=FALSE, message=F, fig.cap='Antwoorden op de vraag: Vindt u dat er voor abortus gekozen mag worden als het leven van de vrouw in gevaar is?'}
...
ggplot(data = vrouw_0_plot_data, mapping = aes(x = percentage, y =Var1)) +
  ...
  ylab(" ") + 
   theme_minimal()
```
Zoals op figuur 5 afgebeeld is neigen mensen eerder naar het toestaan van een abortus wanneer het leven van de vrouw in gevaar is. Dit is veel minder het geval wanneer het om een niet levensbedreigende reden gaat zoals op figuur 7 zichtbaar is. 
...
ggplot(data = veroorzaakt_o_plot_data, mapping = aes(x = percentage, y =Var1)) +
  ...
  theme_minimal()
```
Vergelijkbaar met de vraag of de abortus toegestaan zou moeten worden wanneer het leven van de vrouw in gevaar is kiezen mensen eerder voor de mogelijkheid om voor een abortus te kiezen.   
```{r echo, warning=FALSE, message=F, fig.cap='Antwoorden op de vraag:Vindt u dat er voor abortus gekozen mag worden als de vrouw niet in staat is voor het kind te zorgen vanwege financiele beperkingen?'}
...
as.data.frame(x)%>%
    ...
    theme_minimal()
```
In het bovenstaande plot is de verdeling weergegeven van mensen die wel geloven en niet geloven en wat deze hebben geantwoord op de vraag of mensen zelf mogen kiezen of ze een abortus willen.
...
```{r VisualizatieLR, warning=FALSE,results = "hide", eval=FALSE, fig.cap='Logistische regressie visualisatie voor de vraag: Moet de biologische vader zeggenschap hebben over de beslissing voor een abortus?'}
# Lr
...
ggplot(data = lr_voorspelling_vader, aes(x = rank, y = lr_man_vader.fitted.values)) +
  ...
  theme_minimal()
```
De uitkomst van de logistische regressie is dat leeftijd een significante factor is (p-waarde: 0.00153), geloof bijna met een p-waarde van: 0.05366. \
De trend in leeftijd is ook zichtbaar op de visualisatie in het logboek, de leeftijd van de mensen wordt weergeven met punt grootte. De "index" betekend dat de uitkomsten van de LR gesorteerd zijn van klein naar groot, daarna wordt met behulp van punt kleur weergeven wat de respondent daadwerkelijk gekozen heeft.
De visualisaties van de logistische regressies zijn weergeven in het volgende logboek: Logboek_Floris.
```{r}
